...
Run_number = 1;    
Num_trials = 150; %from 1 to 80 trials   
...
%config_display(1,3,[0.7059 0.7059 0.7059],[.2 .2 .9], 'Arial', 25, 11);
...
StartScreen = 9;
InstructScreen=10;
ConnectionScreen = 11; % SGM2016
BreakScreen = 12; %TV: Mar 2023
...
preparestring('The REAL EXPERIMENT',StartScreen,0,30);
...
preparestring('Connecting to other player',ConnectionScreen,0,30);
preparestring('Get ready to start!',ConnectionScreen,0,0);
preparestring('Good luck!',ConnectionScreen,0,-30);
drawpict(ConnectionScreen);
waituntil( time + 1000*15 );

%%%%% other player now connected.
...
if 0 % USE ORIGINAL? 
   ...
   Event_onsets = (Onsets(:,:,Run_number+1,S_ID)+virtual_cogent_start)*1000; 
    ...
    Onsets = bsxfun(@plus, [2,7], [0:Num_trials-1]'*9 );
    % should give
    %    3   5
    %   12  14
    %   21  23
...
for i = 1:Num_trials
    ...
    %Add a break halfway through the task
    if i == ceil(Num_trials / 2) %TV: divide number of trials by 2 and round up if not a whole number
        clearpict(); %clear background buffer
        preparestring('Halfway! Both Players now have a short 30 seconds break. Get ready to begin again soon.'); %TV: prepare text in default backbuffer (0)
        drawpict(); %TV: draw and flip backbuffer (0) text to screen
        wait(30000); %wait 30 seconds to continue
    end
